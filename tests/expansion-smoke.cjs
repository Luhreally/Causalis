...
const fixtureSource = String.raw`(() => {
  ...
  // ── A crowded town in famine is a place of its own in the count ──
  // The line is the settlers' own; the fixture's town is under it, so the line
  // is lowered to one for the reading and put back. Famine is the granary's
  // reading (82): a larder under five or two in five of the town hungry, and
  // the fixture's ground forages too well for an empty store to read famine,
  // so the town is made hungry instead.
  {
    ...
    settlerLine = () => 1;
    const fedLean = !!foodOutlook(settlement)?.famine;
    out.allowedFed = [ex.allowed(), ex.famineCrowded(), fedLean];
    for (const id of near) if (W.components.life[id]) W.components.life[id].hunger = 95;
    out.allowedLean = [ex.allowed(), ex.famineCrowded(), !!foodOutlook(settlement)?.famine];
    if (!out.allowedLean[2]) fail("a hungry town does not read famine: " + JSON.stringify(foodOutlook(settlement)));
    if (!fedLean && out.allowedLean[1] !== out.allowedFed[1] + 1) fail("a crowded town in famine is not counted once: " + out.allowedFed.join("/") + " -> " + out.allowedLean.join("/"));
    if (!fedLean && out.allowedLean[0] !== out.allowedFed[0] + 1) fail("a crowded town in famine did not open exactly one place: " + out.allowedFed[0] + " -> " + out.allowedLean[0]);
    settlerLine = settlerLineBase;
    for (const [id, h] of hungers) if (W.components.life[id] && h != null) W.components.life[id].hunger = h;
    if (!fedLean && ex.famineCrowded() !== out.allowedFed[1]) fail("the town was not put back: " + ex.famineCrowded());
  }
