...
(async () => {
  ...
  rt.get(`(() => {
    ...
    return 1; })()`);
  const snapshot = () => JSON.parse(rt.get(`JSON.stringify(Object.fromEntries(W.activeIds.filter((id) => W.kind[id] === KINDS.PERSON && classifyAlive(id)).map((id) => [id, W.components.social[id]?.homePlaceId || 0])))`));
  const names = JSON.parse(rt.get(`JSON.stringify(Object.fromEntries(W.settlements.map((s) => [s.id, s.name.slice(0, 8)])))`));
  let before = snapshot();
  for (let n = 0; n < years; n++) {
    rt.get(`(() => { globalThis.__moves = {}; for (let i = 0; i < ${year}; i++) simTick(); return 1; })()`);
    const after = snapshot(), changes = {}, births = [];
    for (const [id, home] of Object.entries(after)) {
      if (!(id in before)) { births.push(names[home] || home); continue; }
      if (before[id] !== home) { const k = (names[before[id]] || before[id] || "none") + "->" + (names[home] || home || "none"); changes[k] = (changes[k] || 0) + 1; }
    }
    const moves = JSON.parse(rt.get("JSON.stringify(globalThis.__moves)"));
    const pops = JSON.parse(rt.get(`JSON.stringify(W.settlements.filter((s) => !s.ruined && s.knownProcesses).map((s) => s.name.slice(0, 8) + ":" + settlementPopulation(s)))`));
    console.log(`y${JSON.parse(rt.get("Math.floor(W.tick / TICKS_PER_YEAR)"))} pops ${pops.join(" ")}\n     home changes ${JSON.stringify(changes)} newcomers ${JSON.stringify(births)}\n     mechanisms ${JSON.stringify(moves)}`);
    before = after;
  }
