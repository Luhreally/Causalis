...
const rt = loadRuntime(), source = process.argv[2] || "tests/fixtures/launch-battery.json.gz", years = Number(process.argv[3] || 6), presses = Number(process.argv[4] ?? 2);
...
(async () => {
  ...
  } else { rt.game.createTestWorld({ seed: source, size: "battery", complexity: "lean" }); const tick = rt.get("simTick"); for (let i = 0; i < year * 30; i++) tick(); }
  ...
  for (let n = 0; n < years; n++) { const r = JSON.parse(rt.get(sample)); console.log(`y${r.year} ppl${r.people} hunger% [0-20,20-40,40-60,60-80,80+] = ${JSON.stringify(r.hunger)} tasks ${JSON.stringify(r.tasks)}`); for (const t of r.towns) console.log("     " + t); }
