...
const seed = process.argv[2] || "7",
  years = Number(process.argv[3] || 200),
  size = process.argv[4] || "standard";
game.createTestWorld({ seed, size });
const simTick = get("simTick"),
  W = probe.W(),
  TPY = get("TICKS_PER_YEAR"),
  KINDS = get("KINDS");
const firstKnown = {},
  t0 = performance.now();
...
outer: for (let y = 1; y <= years; y++) {
  ...
  if (y % 10 === 0) {
    ...
    row.y = y;
    row.techs = known.size;
    row.msTick = +((performance.now() - lastReport) / (TPY * 10)).toFixed(1);
...
console.log(
  "firstKnown",
  JSON.stringify(Object.fromEntries(Object.entries(firstKnown).sort((a, b) => a[1] - b[1]))),
);
console.log("ages", JSON.stringify((W.ages || []).map((a) => [a.gloss, Math.floor(a.tick / TPY)])));
console.log(
  `== ${seed} ${size} ${years} yrs · ${((performance.now() - t0) / (W.tick || 1)).toFixed(2)} ms/tick avg · ${Math.round((performance.now() - t0) / 60000)} min`,
);
