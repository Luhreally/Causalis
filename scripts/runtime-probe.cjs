...
const PROJECT = process.env.CAUSALIS_ROOT || path.resolve(__dirname, "..");
...
class ClassList {
  ...
  toggle(v, force) {
    const n = force == null ? !this.values.has(v) : !!force;
    n ? this.values.add(v) : this.values.delete(v);
    return n;
  }
}
function makeSandbox() {
  ...
  const document = {
    ...
    querySelector(s) {
      return s.startsWith("#") ? getElement(s.slice(1)) : new Element();
    },
    querySelectorAll() {
      return [];
    },
