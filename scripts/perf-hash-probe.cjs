...
const fixture = process.argv[2] || path.join(__dirname, "..", "tests", "fixtures", "launch-battery.json.gz");
...
(async () => {
  ...
  if (!(await rt.sandbox.window.ALIFE_SAVE_DEBUG.load("perf"))) throw new Error("the fixture did not load");
  ...
  for (const key of ["events", "historicalIdentities", "annals", "components", "tiles", "settlements", "activeIds", "cohorts", "factions", "buildings"])
    rows.push(JSON.parse(time(`without ${key}`, `const saved = W[${JSON.stringify(key)}]; W[${JSON.stringify(key)}] = Array.isArray(saved) ? [] : {};`, `W[${JSON.stringify(key)}] = saved;`)));
  rows.push(JSON.parse(time("whole world again")));
  const whole = rows[0].ms;
  for (const r of rows) console.log(`${String(r.ms).padStart(7)} ms  ${r.label.padEnd(30)} ${r.label.startsWith("without") ? `(${(whole - r.ms).toFixed(1)} ms is that member)` : r.hash}`);
  const counts = JSON.parse(rt.get(`JSON.stringify({ events: W.events.length, identities: Object.keys(W.historicalIdentities || {}).length, annals: (W.annals || []).length, active: W.activeIds.length, strings: (() => { let n = 0, chars = 0; for (const e of W.events) for (const s of e.evidence || []) { n++; chars += String(s).length; } return { n, chars }; })() })`));
