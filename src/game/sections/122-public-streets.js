...
function publicRoutes() {
  return W.publicTransport?.routes || [];
}
...
drawWornPath = function (x, y, i, p, m, v) {
  ...
  const town = nearestSettlement(i, 7),
    urban = town && cityStage(town),
    motor = urban && knowsTech(town, "combustion");
  ...
  for (const [dx, dy] of [
    ...
    const nx = x + dx,
      ny = y + dy;
    if (inside(nx, ny) && roadLevel(idx(nx, ny)) === ROAD_PAVED)
  ...
  stroke(
    motor ? hsl(205, 9, 28 - wear * 10) : hsl(35, 11, 47 - wear * 9),
    m.tw * (0.28 + wear * 0.04),
  );
  if (motor && UI.camera.zoom > 1.5) {
    ctx.setLineDash([Math.max(1, m.tw * 0.13), Math.max(1, m.tw * 0.12)]);
    stroke("#e8d9a2", Math.max(0.65, m.tw * 0.015));
    ctx.setLineDash([]);
    if (segments.length >= 3) {
      ctx.strokeStyle = hsl(44, 22, 88 + wear * 6);
      ctx.lineWidth = Math.max(0.65, m.tw * (0.025 + wear * 0.012));
      for (let n = -2; n <= 2; n++) {
        ctx.beginPath();
        ctx.moveTo(p.x + n * m.tw * 0.04, p.y - m.th * 0.1);
        ctx.lineTo(p.x + n * m.tw * 0.04, p.y + m.th * 0.1);
        ctx.stroke();
      }
    }
  }
...
drawWorkerActivity = function (now, bounds) {
  ...
  for (const route of publicRoutes()) {
    ...
    const p = proceduralProjectTile(x + 0.5, y + 0.5, m),
      r = Math.max(2, m.tw * 0.17);
    ctx.save();
    ctx.translate(p.x, p.y);
    ctx.scale(route.direction, 1);
    const hue = visualHash01(link.id, 0xb051) * 360;
    ctx.fillStyle = hsl(hue, 40, 48);
    ctx.fillRect(-r * 1.8, -r * 1.1, r * 3.6, r);
    ctx.fillStyle = "#a9d8df";
    for (let n = 0; n < 5; n++)
      ctx.fillRect(-r * 1.55 + n * r * 0.62, -r * 0.95, r * 0.45, r * 0.38);
    ctx.fillStyle = "#22272a";
    for (const dx of [-1.1, 1.1]) {
      ctx.beginPath();
      ctx.arc(dx * r, -r * 0.05, r * 0.23, 0, Math.PI * 2);
      ctx.fill();
    }
    if (route.kind === "tram") {
      ctx.strokeStyle = "#c3c8c9";
      ctx.beginPath();
      ctx.moveTo(0, -r * 1.1);
    ...
    for (const at of [0, link.path.length - 1]) {
      const [sx, sy] = xy(link.path[at]),
        stop = proceduralProjectTile(sx + 0.65, sy + 0.65, m);
      ctx.fillStyle = "#364957";
      ctx.fillRect(stop.x, stop.y - r * 0.9, Math.max(1, r * 0.1), r * 0.9);
      ctx.fillStyle = "#e9d9a6";
      ctx.fillRect(stop.x - r * 0.2, stop.y - r * 1.15, r * 0.5, r * 0.4);
    }
  }
};
