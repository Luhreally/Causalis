...
function habitationBeds(b) {
  return b && b.complete && !b.ruined && !b.abandoned && HABITATION_TYPES.has(b.type)
    ? Math.max(0, b.housing || BUILDING_DEFS[b.type]?.housing || 0)
    : 0;
}
function habitationHome(id) {
  const social = W.components.social[id],
    home = social?.homeBuildingId;
  return home && social.homePlaceKind === "settlement"
    ? W.buildings.find(
        (b) => b.id === home && b.placeId === social.homePlaceId && habitationBeds(b) > 0,
      ) || null
    : null;
}
  ...
  return W.activeIds
    .filter(
      (id) =>
        ...
        W.components.social[id]?.homePlaceId === town.id,
    )
    .sort((a, b) => a - b);
}
...
function habitationFamilies(town, residents) {
  ...
  for (const id of residents) {
    ...
    const group = [id],
      partner = W.components.social[id]?.partnerId;
...
function updateHabitationTown(town) {
  const residents = habitationResidents(town),
    ...
    homes = W.buildings
      .filter((b) => b.placeKind === "settlement" && b.placeId === town.id && habitationBeds(b) > 0)
      .sort(
        (a, b) => (HABITATION_FILL[a.type] ?? 3) - (HABITATION_FILL[b.type] ?? 3) || a.id - b.id,
      ),
  ...
  for (const b of homes) {
    ...
    if (b.tenancy.ownerId && !classifyAlive(b.tenancy.ownerId)) {
      const heirs = (W.components.identity[b.tenancy.ownerId]?.children || []).filter((id) =>
        living.has(id),
      );
  ...
  for (const group of habitationFamilies(town, residents)) {
    ...
    const mayTake = (b) => b === old || habitationMayTake(town, b, group),
      together =
        old && room(old) >= group.length
          ? old
          : homes.find((b) => room(b) >= group.length && mayTake(b));
    for (const id of group) {
      const b = together || homes.find((h) => room(h) > 0 && mayTake(h)),
  ...
  for (const b of homes) {
    ...
    for (const id of Object.keys(b.tenancy.arrears))
      if (!b.tenancy.residents.includes(+id)) delete b.tenancy.arrears[id];
  }
  town.habitation = {
    residents: residents.length,
    beds: homes.reduce((n, b) => n + habitationBeds(b), 0),
...
function habitationEvict(town, homes) {
  const social = W.components.social,
    ...
    purse = (h) =>
      seeking
        .filter((id) => social[id].householdId === h)
        .reduce((n, id) => n + Math.max(0, W.components.identity[id]?.civicCoins || 0), 0),
  ...
  for (const b of homes) {
    ...
    if (b.type === "shelter" || !b.tenancy || b.tenancy.residents.length < habitationBeds(b))
      continue;
    ...
    const payer = payers.splice(
        payers.findIndex((h) => purse(h) >= rent),
        1,
      )[0],
  ...
  for (const b of homes) {
    ...
    const inside = (h) => t.residents.filter((id) => social[id]?.householdId === h),
      hopeless = Object.entries(t.arrears)
        .filter(
          ([h, n]) =>
            n >= HABITATION_HOPELESS_ARREARS * habitationRentOf(t, +h) &&
            inside(+h).length &&
            !inside(+h).some((id) => (W.components.identity[id]?.civicCoins || 0) > 0),
        )
        .sort((x, y) => y[1] - x[1] || +x[0] - +y[0])[0];
    if (!hopeless) continue;
    const head = +hopeless[0],
      out = inside(head);
    for (const id of out) social[id].homeBuildingId = 0;
...
function habitationAccounts(town, homes) {
  ...
  if (faction && polityCoins(faction)) {
    ...
    for (const b of homes) {
      const tenancy = b.tenancy,
        owner = W.components.identity[tenancy.ownerId],
        heads = [...new Set(tenancy.residents.map((id) => W.components.social[id].householdId))];
      tenancy.rents = {};
      for (const id of heads) {
        if (
          tenancy.residents.some(
            (x) => x === tenancy.ownerId && W.components.social[x].householdId === id,
          )
        )
          continue;
        ...
        for (const m of habitationPurse(id, members)) {
          const ident = W.components.identity[m],
            take = Math.min(due - paid, Math.max(0, ident.civicCoins || 0));
    ...
    for (const b of homes) {
      ...
      if (!tenancy.ownerId) {
        const asking = habitationPrice(b, town),
          buyer = tenancy.residents.find(
            (id) => (W.components.identity[id]?.civicCoins || 0) >= asking,
          );
        if (buyer) {
          const price = asking;
  ...
  for (const b of homes) {
    ...
    inv[C.FIBER]--;
    inv[C.PIGMENT]--;
    b.composition[C.FIBER]++;
    b.composition[C.PIGMENT]++;
    const kinds = cityKnows(town, "writing")
      ? ["woven rug", "wall picture", "bookshelf", "cushions"]
      : ["woven rug", "wall picture", "cushions"];
    b.tenancy.decor.push({
      kind: kinds[hashParts(W.seedHash, maker, b.id, year) % kinds.length],
      maker,
      year,
...
tickSystem("habitation", function () {
  ...
  for (const town of W.settlements)
    if (!town.ruined) habitationAccounts(town, updateHabitationTown(town));
});
...
personMayEnterBuilding = function (id, b) {
  if (
    b?.tenancy &&
    HABITATION_TYPES.has(b.type) &&
    ["rest", "return"].includes(W.components.life[id]?.behavior)
  )
...
makeInteriorState = function (bounds) {
  ...
  for (const b of W.buildings)
    if (b.tenancy) {
      ...
      const residents = b.tenancy.residents.filter(
...
function habitationFloorPlan(g, b, s, r, p, homes) {
  ...
  for (let n = 0; n < units; n++) {
    ...
    if (item.kind === "woven rug")
      g.fillRect(x + unitW * 0.2, top + h * 0.74, unitW * 0.6, h * 0.16);
...
drawInteriorFurniture = function (g, b, s, r, p, homes) {
  if (HABITATION_TYPES.has(b.type) && b.type !== "shelter")
    return habitationFloorPlan(g, b, s, r, p, homes);
  drawInteriorFurnitureHabitationBase(
    g,
    HABITATION_TYPES.has(b.type) ? { ...b, type: "shelter" } : b,
    s,
    r,
    p,
    homes,
  ...
  for (const [n, item] of (b.tenancy?.decor || []).slice(-4).entries()) {
    const x = s.x - r * 0.65 + n * r * 0.4,
      y = s.y - r * 0.65;
    g.fillStyle = hsl(item.hue, 48, 58);
    if (item.kind === "woven rug") {
      g.fillRect(x, s.y + r * 0.22, r * 0.3, r * 0.18);
      g.strokeStyle = p.light;
      g.strokeRect(x + r * 0.03, s.y + r * 0.25, r * 0.24, r * 0.12);
    } else {
      g.fillRect(x, y, r * 0.26, r * 0.2);
      g.strokeStyle = p.dark;
      g.strokeRect(x, y, r * 0.26, r * 0.2);
      if (item.kind === "bookshelf")
        for (let j = 0; j < 4; j++) {
          g.fillStyle = hsl(item.hue + j * 36, 40, 48);
          g.fillRect(x + r * j * 0.06, y + r * 0.02, r * 0.035, r * 0.15);
        }
    }
  }
};
    ...
    button.onclick = () => {
      const tallest = W
        ? Math.max(
            1,
            ...W.buildings
              .filter((b) => ["tower", "tenement", "office"].includes(b.type))
              .map(blockStoreys),
          )
        : 1;
...
drawBuildingInterior = function (g, b, now, m, state) {
  if (!["tower", "tenement", "office"].includes(b.type))
    return drawBuildingInteriorHabitationBase(g, b, now, m, state);
  const floors = blockStoreys(b),
    floor = Math.min(floors - 1, UI.camera.interiorFloor || 0),
    homes = state.homes.get(b.id) || [],
    perFloor = Math.max(1, Math.ceil(habitationBeds(b) / floors)),
    onFloor = homes.slice(floor * perFloor, (floor + 1) * perFloor),
    inside = (state.inside.get(b.id) || []).filter((id) =>
      homes.includes(id) ? onFloor.includes(id) : id % floors === floor,
    ),
    local = { ...state, homes: new Map(state.homes), inside: new Map(state.inside) };
  local.homes.set(b.id, onFloor);
  local.inside.set(b.id, inside);
  ...
  if (r < 26) return;
  g.save();
  g.font = `${Math.max(9, Math.min(13, r * 0.28))}px sans-serif`;
  g.textAlign = "center";
  g.fillStyle = "#f2e8d4";
...
renderLifePage = function (id) {
  ...
  return (
    html +
    (b
      ? `<div class="kv"><span>Home</span><b>${esc(b.name || b.type)} · ${b.tenancy?.ownerId === id ? "owner" : "resident"}</b></div>`
      : "") +
    (ident?.civicCoins != null
      ? `<div class="kv"><span>Household savings</span><b>${ident.civicCoins} coin</b></div>`
      : "")
  );
};
window.ALIFE_HABITATION_DEBUG = Object.freeze({
  update: () => {
    for (const s of W.settlements) if (!s.ruined) habitationAccounts(s, updateHabitationTown(s));
  },
  home: (id) => habitationHome(id)?.id || 0,
  homes: () =>
    W.buildings
      .filter((b) => habitationBeds(b))
      .map((b) => ({ id: b.id, capacity: habitationBeds(b), ...b.tenancy })),
  families: () => ({ ...HABITATION }),
  evict: (placeId) => {
