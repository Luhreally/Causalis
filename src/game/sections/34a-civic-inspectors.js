...
refreshWorldInfo = function () {
  ...
  const c = civilizationSummary(),
    ...
    card = `<div class="subhead">Cell-to-civilization progression</div><div class="card"><div class="row between"><b>${esc(titleCase(c.stage))}</b><span class="tag gold">last stage on the ground: ${esc(c.endpoint)}</span></div><div class="row wrap" style="margin-top:9px">${fullStages.map((stage, n) => `<span class="tag ${n <= current ? "good" : ""}">${n <= current ? "✓ " : ""}${esc(titleCase(stage))}</span>`).join("")}</div><div class="kv" style="margin-top:9px"><span>Next gated transition</span><b>${esc(next ? titleCase(next) : "Endpoint achieved")}</b>${gate ? `<span>Gate requirement</span><b>${esc(gate.requirement)}</b><span>Closest candidate</span><b>${gate.leader ? `${esc(gate.leader)}${gate.missing.length ? ` — still needs: ${esc(gate.missing.join(" · "))}` : " — requirements met"}` : esc(gate.missing.join(" · ") || "requirements met")}</b>` : ""}<span>World buildings (all places)</span><b>${c.buildings.complete} complete / ${c.buildings.planned} active</b><span>Functional tools</span><b>${c.tools.count}</b><span>LTC decisions influenced</span><b>${fmt(c.cognition.influences)}</b><span>Milestones recorded</span><b>${c.milestones.length}</b></div><div class="divider"></div><small class="muted">Cellular and multicellular emergence remain evolutionary. Tribal, village, civic, urban and complex terrestrial stages require real tools, structures, population, institutions and research. Past them, a modern world can build a ship and send it to the stars (the Causal skip names what each world still lacks).</small></div>`;
