...
refreshWorldInfo = function () {
  ...
  const v = makePlanetVisualGenome(),
    g = v.terrain,
    card = `<div class="subhead">Procedural planet genome</div><div class="card"><div class="row between"><b>${v.earthlike ? "Earth-adjacent baseline" : `${titleCase(g.topology)} alien world`}</b><span class="tag">${Math.round(v.alienness * 100)}% alien</span></div><div class="kv" style="margin-top:8px"><span>Relief grammar</span><b>${titleCase(g.topology)}</b><span>Climate geometry</span><b>${titleCase(g.climate)}</b><span>Landforms</span><b>${landformSummary(g)}</b><span>Hydrology</span><b>${landformHydrologySummary(g)}</b><span>Seasons</span><b>${g.landform?.season?.amplitude ? `${(g.landform.season.amplitude / 5).toFixed(0)}° swing` : "none (untilted)"}</b><span>First tongue</span><b>${esc(firstTongueSummary())}</b><span>Solvent coverage</span><b>${pct(g.targetWater * 100)}</b><span>Producer morphology</span><b>${titleCase(v.floraForm)}</b><span>Geology motif</span><b>${titleCase(v.geologyForm)}</b><span>Solvent surface</span><b>${titleCase(v.waterForm)}</b>${v.surface ? `<span>Surface grain</span><b>${titleCase(v.surface.grain)}</b><span>Undergrowth</span><b>${titleCase(v.floraForm)} and ${titleCase(v.surface.floraFormAlt)}</b><span>Prevailing wind</span><b>${Math.round(((v.surface.wind.angle * 180) / Math.PI + 360) % 360)}° at ${v.surface.wind.speed.toFixed(1)}×</b><span>Sky</span><b>${titleCase(v.surface.clouds.form)} cloud${v.surface.sky.aurora ? " · aurora" : ""}</b><span>Airborne</span><b>${v.surface.particle.kind === "none" ? "clear air" : titleCase(v.surface.particle.kind)}</b>` : ""}</div><div class="row" style="margin-top:9px"><span class="dot" style="color:${hsl(v.liquidHue, 65, 52)}"></span><span class="dot" style="color:${hsl(v.floraHue, 62, 52)}"></span><span class="dot" style="color:${hsl(v.mineralHue, 52, 57)}"></span><span class="dot" style="color:${hsl(v.accentHue, 70, 62)}"></span><small class="muted">Palette derived from this world's solvent, pigment, energy, and mineral compounds</small></div></div>`;
